...
class DuesModelDues extends JModelList
{
	...
	public function batchProcess($batch_year)
	{
		function getActiveMembers()
		{
			...
			$query->where('(' . $db->quoteName('a.cb_memberlevel') . ' != "Non-Member User" AND (' . $db->quoteName('a.cb_memberstatus') . ' = "Active" OR ' . $db->quoteName('a.cb_memberstatus'). ' = "Affiliate"))');
			$db->setQuery($query);
		...
		if($limit_reached){
			...
			JFactory::getApplication()->enqueueMessage(JText::_('COM_DUES_WARNING_BATCH_DONE'));
		}
		return true;
	}
}
